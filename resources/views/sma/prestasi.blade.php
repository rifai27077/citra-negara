...
@section('title', 'Prestasi SMA Citra Negara')
@section('meta_description', 'Kumpulan prestasi dan pencapaian siswa SMA Citra Negara di berbagai bidang akademik dan non-akademik.')

@section('content')
<section id="all-prestasi" class="relative py-24 px-6 bg-[#f7f7f7] overflow-hidden">
  ...
  <div class="max-w-7xl mx-auto relative z-10">
    @php
        $prestasi = \App\Models\Prestasi::where('category', 'SMA')->latest('date')->paginate(9);
    @endphp
    ...
    <!-- <nav class="flex items-center text-sm text-gray-500 mb-8" aria-label="Breadcrumb">
      ...
      <span class="text-gray-900 font-medium">Prestasi</span>
    </nav> -->

    <!-- Judul Halaman -->
    <div class="text-center mb-14">
      <h1 class="text-3xl md:text-5xl font-extrabold text-[#2F2F2F] leading-tight">
        Prestasi <span class="text-[#699D15]">SMA</span> Citra Negara
      </h1>
      <p class="text-gray-600 text-base md:text-lg mt-5 max-w-3xl mx-auto leading-relaxed">
        Catatan kebanggaan dan dedikasi siswa SMA Citra Negara dalam meraih prestasi di tingkat lokal, nasional, hingga internasional.
      </p>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-10" data-aos="fade-up">
      @forelse ($prestasi as $item)
        <div class="group relative rounded-2xl overflow-hidden bg-white shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col h-full">
          ...
          <div class="p-8 flex flex-col flex-grow">
            ...
            <a href="{{ route('sma.prestasi.show', $item->slug) }}" 
      ...
      @empty
        <div class="col-span-3 text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
            <p class="text-gray-400 text-lg">Belum ada data prestasi SMA.</p>
        </div>
      @endforelse
    </div>
    ...
    <div class="text-center mt-16">
      <a href="{{ route('sma.index') }}" class="inline-flex items-center text-gray-600 hover:text-[#699D15] font-semibold transition-colors">
        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
        Kembali ke Beranda SMA
      </a>
    </div>
